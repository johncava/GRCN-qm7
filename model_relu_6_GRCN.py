...
for fold in folds:
    ...
    for epoch in xrange(1,epochs):
        for iteration in xrange(1,len(fold)/batch):
            ...
            hidden_layer_6 = F.leaky_relu(D.bmm(A).bmm(D).bmm(hidden_layer_5).bmm(W6) + hidden_layer_3)
...
plt.title('GRCN 6 Layer with leaky_relu (lr=1e-1)')
plt.show()
plt.savefig('results_relu_6layer_GRCN_lr=1e-1.png')
