...
for fold in folds:
    ...
    W10 = Variable(torch.randn(23*5, 1).type(FloatTensor), requires_grad=True)
    ...
    for epoch in xrange(1,epochs):
        for iteration in xrange(1,len(fold)/batch):
            ...
            y_pred = F.leaky_relu(D.bmm(A).bmm(D).bmm(hidden_layer_8).bmm(W9))
            y_pred = y_pred.view(batch, 23*5)
            #y_pred = 2000*F.sigmoid(y_pred.mm(W4))
            y_pred = F.leaky_relu(y_pred.mm(W10))
...
plt.title('GRCN 8 Layer with leaky_relu')
plt.show()
plt.savefig('results_relu_8layer_GRCN.png')
