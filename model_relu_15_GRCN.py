...
for fold in folds:
    ...
    W5 = Variable(torch.randn(batch, 23, 23).type(FloatTensor), requires_grad=True)
    W6 = Variable(torch.randn(batch, 23, 23).type(FloatTensor), requires_grad=True)
    W7 = Variable(torch.randn(batch, 23, 23).type(FloatTensor), requires_grad=True)
    W8 = Variable(torch.randn(batch, 23, 23).type(FloatTensor), requires_grad=True)
    W9 = Variable(torch.randn(batch, 23, 5).type(FloatTensor), requires_grad=True)
    W10 = Variable(torch.randn(115, 115).type(FloatTensor), requires_grad=True)
    W11 = Variable(torch.randn(115, 115).type(FloatTensor), requires_grad=True)
    W12 = Variable(torch.randn(115, 115).type(FloatTensor), requires_grad=True)
    W13 = Variable(torch.randn(115, 115).type(FloatTensor), requires_grad=True)
    W14 = Variable(torch.randn(115, 115).type(FloatTensor), requires_grad=True)
    W15 = Variable(torch.randn(115, 1).type(FloatTensor), requires_grad=True)
    ...
    for epoch in xrange(1,epochs):
        for iteration in xrange(1,len(fold)/batch):
            ...
            hidden_layer_3 = F.leaky_relu(D.bmm(A).bmm(D).bmm(hidden_layer_2).bmm(W3) + hidden_layer_1)
            hidden_layer_4 = F.leaky_relu(D.bmm(A).bmm(D).bmm(hidden_layer_3).bmm(W4))
            hidden_layer_5 = F.leaky_relu(D.bmm(A).bmm(D).bmm(hidden_layer_4).bmm(W5))
            hidden_layer_6 = F.leaky_relu(D.bmm(A).bmm(D).bmm(hidden_layer_5).bmm(W6) + hidden_layer_2)
            hidden_layer_7 = F.leaky_relu(D.bmm(A).bmm(D).bmm(hidden_layer_6).bmm(W7))
            hidden_layer_8 = F.leaky_relu(D.bmm(A).bmm(D).bmm(hidden_layer_7).bmm(W8))
            hidden_layer_9 = F.leaky_relu(D.bmm(A).bmm(D).bmm(hidden_layer_8).bmm(W9) + D.bmm(A).bmm(D).bmm(hidden_layer_6).bmm(W9))
            hidden_layer_9 = hidden_layer_9.view(batch, 23*5)
            #y_pred = 2000*F.sigmoid(y_pred.mm(W4))
            hidden_layer_10 = F.leaky_relu(hidden_layer_9.mm(W10))
            hidden_layer_11 = F.leaky_relu(hidden_layer_10.mm(W11))
            hidden_layer_12 = F.leaky_relu(hidden_layer_11.mm(W12) + hidden_layer_10)
            hidden_layer_13 = F.leaky_relu(hidden_layer_12.mm(W13))
            hidden_layer_14 = F.leaky_relu(hidden_layer_13.mm(W14))
            y_pred = F.leaky_relu(hidden_layer_14.mm(W15))
...
plt.title('GRCN 15 Layer with leaky_relu (lr = 1e-5)')
plt.show()
plt.savefig('results_relu_15layer_GRCN_lr=1e-5.png')
