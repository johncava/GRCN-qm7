...
for fold in folds:
    ...
    W7 = Variable(torch.randn(batch, 23, 5).type(FloatTensor), requires_grad=True)
    W8 = Variable(torch.randn(23*5, 1).type(FloatTensor), requires_grad=True)
    ...
    for epoch in xrange(1,epochs):
        for iteration in xrange(1,len(fold)/batch):
            ...
            hidden_layer_1 = F.sigmoid(D.bmm(A).bmm(D).bmm(x).bmm(W1))
            hidden_layer_2 = F.sigmoid(D.bmm(A).bmm(D).bmm(hidden_layer_1).bmm(W2))
            hidden_layer_3 = F.sigmoid(D.bmm(A).bmm(D).bmm(hidden_layer_2).bmm(W3))
            hidden_layer_4 = F.sigmoid(D.bmm(A).bmm(D).bmm(hidden_layer_3).bmm(W4))
            hidden_layer_5 = F.sigmoid(D.bmm(A).bmm(D).bmm(hidden_layer_4).bmm(W5))
            hidden_layer_6 = F.sigmoid(D.bmm(A).bmm(D).bmm(hidden_layer_5).bmm(W6))
            y_pred = F.leaky_relu(D.bmm(A).bmm(D).bmm(hidden_layer_6).bmm(W7))
            y_pred = y_pred.view(batch, 23*5)
            #y_pred = 2000*F.sigmoid(y_pred.mm(W4))
            y_pred = F.leaky_relu(y_pred.mm(W8))
...
plt.title('GCN 6 Layer with sigmoid (lr=1e-1)')
plt.show()
plt.savefig('results_sigmoid_6layer_GCN_lr=1e-1.png')
